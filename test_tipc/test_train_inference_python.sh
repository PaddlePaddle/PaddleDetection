        ...
        for autocast in ${autocast_list[*]}; do
            for trainer in ${trainer_list[*]}; do
                ...
                if [ ${#gpu} -le 2 ];then  # train with cpu or single gpu
                    ...
                    IFS="|"
                    set_save_model=$(func_set_params "${save_model_key}" "${save_log}")
                    cmd="${python} -m paddle.distributed.launch --ips=${ips} --devices=${gpu} ${run_train} log_iter=1 ${set_use_gpu} ${set_save_model} ${set_epoch} ${set_pretrain} ${set_batchsize} ${set_filename} ${set_shuffle} ${set_amp_level} ${set_enable_ce} ${set_autocast} ${set_to_static} ${set_train_params1}"
                fi
                # run train
