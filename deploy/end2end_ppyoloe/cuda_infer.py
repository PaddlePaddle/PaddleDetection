...
class PPYOLOE:
    ...
    def __init__(self, engine_path: str):
        ...
        for binding in engine:
            shape = trt.volume(engine.get_binding_shape(binding))
            dtype = trt.nptype(engine.get_binding_dtype(binding))
            # Allocate host and device buffers
            host_data = np.empty(shape, dtype=np.dtype(dtype))
            _, device_mem = cudart.cudaMallocAsync(host_data.nbytes, self.stream)
            # Append the device buffer to device bindings.
            self.bindings.append(int(device_mem))
            if engine.binding_is_input(binding):
                self.batch_size, _, self.width, self.height = engine.get_binding_shape(binding)
                self.inputs.append({'host': host_data, 'device': device_mem})
            else:
                self.outputs.append({'host': host_data, 'device': device_mem})
    ...
    def batch_infer(self, images: List[np.ndarray]) -> List[Dict[str, Any]]:
        ...
        for grouped_images in [images[i:i + self.batch_size] for i in range(0, len(images), self.batch_size)]:
            ...
            batch_input_image = np.empty(shape=[self.batch_size, 3, self.width, self.height])
            for idx, image in enumerate(grouped_images):
                ...
                np.copyto(batch_input_image[idx], input_image)
            batch_input_image = np.ascontiguousarray(batch_input_image)

            # Copy input image to host buffer
            np.copyto(self.inputs[0]['host'], batch_input_image.ravel())

            # Transfer data to the gpu
            for inp in self.inputs:
                cudart.cudaMemcpyAsync(inp['device'], inp['host'].data, inp['host'].nbytes, cudart.cudaMemcpyKind.cudaMemcpyHostToDevice, self.stream)
            ...
            for out in self.outputs:
                cudart.cudaMemcpyAsync(out['host'].data, out['device'], out['host'].nbytes, cudart.cudaMemcpyKind.cudaMemcpyDeviceToHost, self.stream)

            # synchronize stream
            cudart.cudaStreamSynchronize(self.stream)

            outputs.extend(self.postprocess([out['host'] for out in self.outputs], batch_image_ratio, batch_image_dwdh))
