...
def image_preprocess_mobilenetv3(img_path,re_shape):
    ...
    img = cv2.resize(img, (re_shape,re_shape), interpolation=cv2.INTER_LANCZOS4)
...
def benchmark(img_file,onnx_file,re_shape):
    ...
    print(f'inference_time(ms): min={round(time_min*1000, 2)}, max = {round(time_max*1000, 1)}, avg = {round(time_avg*1000, 1)}')
...
if __name__=='__main__':
    ...
    parser.add_argument('--img_path', type=str, default='demo/000000570688.jpg', help="image path")
    parser.add_argument('--onnx_path', type=str, default='out_onnxsim/picodet_l_320_processed.onnx', help="onnx filepath")
