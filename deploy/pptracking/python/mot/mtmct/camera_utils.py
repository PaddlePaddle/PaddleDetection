...
def get_sim_matrix(cid_tid_dict, cid_tids, use_ff=True, use_rerank=True):
    count = len(cid_tids)
    ...
    st_mask = st_filter(st_mask, cid_tids, cid_tid_dict) # different from utils.py
...
def get_labels_with_camera(cid_tid_dict, cid_tids, score_thr, use_ff=True, use_rerank=True):
    # 1st cluster
    ...
    for i,sub_c_to_c in enumerate(sub_cid_tids):
        # print('sub_c_to_c => ', sub_c_to_c)
        sim_matrix = get_sim_matrix(cid_tid_dict,sub_cid_tids[sub_c_to_c],use_ff=use_ff,use_rerank=use_rerank)
    ...
    sub_labels = dict()
    for i,sub_c_to_c in enumerate(sub_cid_tids):
        sim_matrix = get_sim_matrix(cid_tid_dict_new,sub_cid_tids[sub_c_to_c],use_ff=use_ff,use_rerank=use_rerank)
