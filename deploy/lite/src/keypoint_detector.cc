...
void KeyPointDetector::LoadModel(std::string model_file,
               int num_theads) {
  MobileConfig config;
...
const int edge[][2] = {{0, 1}, {0, 2}, {1, 3}, {2, 4}, {3, 5}, {4, 6}, {5, 7}, {6, 8},
  ...
  for (int batchid=0; batchid<results.size();batchid++)
  {
    for (int i = 0; i < results[batchid].num_joints; i++) {
      if(results[batchid].keypoints[i*3] > 0.5)
      {
        ...
        cv::circle(vis_img, cv::Point2d(x_coord, y_coord), 1, cv::Scalar(0, 0, 255), 2);
      }
    }
    for (int i=0; i<results[batchid].num_joints; i++)
    {
      ...
      cv::line(vis_img, cv::Point2d(x_start, y_start), cv::Point2d(x_end, y_end), colormap[i], 1);
    }
  }
  return vis_img;
}
...
void KeyPointDetector::Postprocess(
  ...
  {
    get_final_preds(const_cast<float*>(output.data()), output_shape, idxout.data(), idx_shape, center_bs[batchid], scale_bs[batchid], preds, batchid);
...
void KeyPointDetector::Predict(const std::vector<cv::Mat> imgs,
 ...
 auto input_names = predictor_->GetInputNames();
  ...
  for (int i = 0; i < repeats; i++)
  {
    ...
    std::copy_n(out_tensor->mutable_data<float>(), output_size, output_data_.data());
    ...
    std::copy_n(idx_tensor->mutable_data<int64_t>(), output_size, idx_data_.data());
  }
  auto inference_end = std::chrono::steady_clock::now();
  auto postprocess_start = std::chrono::steady_clock::now();
  // Postprocessing result
  Postprocess(output_data_, output_shape, idx_data_, idx_shape, result, center_bs, scale_bs);
