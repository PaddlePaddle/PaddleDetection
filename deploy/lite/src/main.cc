...
void PrintBenchmarkLog(std::vector<double> det_time, int img_num){
  ...
  std::cout << "num_threads: " << RT_Config["cpu_threads"].as<int>() <<std::endl;
  std::cout << "----------------------- Data info -----------------------" <<std::endl;
  std::cout << "batch_size_det: " << RT_Config["batch_size_det"].as<int>() <<std::endl;
  std::cout << "batch_size_keypoint: " << RT_Config["batch_size_keypoint"].as<int>() <<std::endl;
  std::cout << "----------------------- Model info -----------------------" <<std::endl;
  RT_Config["model_dir_det"].as<std::string>().erase(RT_Config["model_dir_det"].as<std::string>().find_last_not_of("/") + 1);
  std::cout << "detection model_name: " << RT_Config["model_dir_det"].as<std::string>().substr(RT_Config["model_dir_det"].as<std::string>().find_last_of('/') + 1) <<std::endl;
  RT_Config["model_dir_keypoint"].as<std::string>().erase(RT_Config["model_dir_keypoint"].as<std::string>().find_last_not_of("/") + 1);
  std::cout << "keypoint model_name: " << RT_Config["model_dir_keypoint"].as<std::string>().substr(RT_Config["model_dir_keypoint"].as<std::string>().find_last_of('/') + 1) <<std::endl;
    ...
    for (int i = 0; i < left_image_cnt; i++) {
      ...
      if(keypoint)
      {
        ...
        for (int i=0; i<imsize; i++){
          ...
          if (imgs_kpts.size()==RT_Config["batch_size_keypoint"].as<int>() || ((i==imsize-1)&&!imgs_kpts.empty()))
          {
            if (run_benchmark) {
...
int main(int argc, char** argv) {
  ...
  PaddleDetection::load_jsonf(std::string("runtime_config.json"), RT_Config);
  if (RT_Config["model_dir_det"].as<std::string>().empty()
      || (RT_Config["image_file"].as<std::string>().empty() && RT_Config["image_dir"].as<std::string>().empty())) {
    std::cout << "Usage: ./main --model_dir_det=/PATH/TO/INFERENCE_MODEL/ (--model_dir_keypoint=/PATH/TO/INFERENCE_MODEL/)"
                << "--image_file=/PATH/TO/INPUT/image.jpg (--image_dir=/PATH/TO/IMAGES/)" << std::endl;
    return -1;
  }
  // Load model and create a object detector
  PaddleDetection::ObjectDetector det(RT_Config["model_dir_det"].as<std::string>(),
                        RT_Config["cpu_threads"].as<int>(), RT_Config["batch_size_det"].as<int>());

  PaddleDetection::KeyPointDetector* keypoint = nullptr;
  if (!RT_Config["model_dir_keypoint"].as<std::string>().empty())
  {
      keypoint = new PaddleDetection::KeyPointDetector(RT_Config["model_dir_keypoint"].as<std::string>(),
                        RT_Config["cpu_threads"].as<int>(), RT_Config["batch_size_keypoint"].as<int>());
      RT_Config["batch_size_det"] = 1;
      printf("batchsize of detection forced to be 1 while keypoint model is not empty()");
  }
  // Do inference on input image

  if (!RT_Config["image_file"].as<std::string>().empty() || !RT_Config["image_dir"].as<std::string>().empty()) {
    ...
    PredictImage(all_img_paths, RT_Config["batch_size_det"].as<int>(), RT_Config["threshold_det"].as<float>(),
		 RT_Config["run_benchmark"].as<bool>(), &det, keypoint, RT_Config["output_dir"].as<std::string>());
  }
  delete keypoint;
  keypoint = nullptr;
  return 0;
