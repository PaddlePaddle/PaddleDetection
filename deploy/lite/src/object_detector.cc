...
void ObjectDetector::LoadModel(std::string model_file,
               int num_theads) {
  MobileConfig config;
...
void ObjectDetector::Predict(const std::vector<cv::Mat> &imgs,
  ...
  for (int i = 0; i < repeats; i++)
  {
    ...
    std::copy_n(output_tensor->mutable_data<float>(), output_size, output_data_.data());
    ...
    std::copy_n(out_bbox_num->mutable_data<int>(), out_bbox_num_size, out_bbox_num_data_.data());
  }
