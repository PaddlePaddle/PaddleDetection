...
void affine_tranform(float pt_x, float pt_y, cv::Mat& trans,float* preds,int p) {
...
void get_affine_transform(std::vector<float>& center, std::vector<float>& scale, float rot, std::vector<int>& output_size, cv::Mat &trans, int inv) {
	float src_w = scale[0];
	...
	dstPoint2f[1] = cv::Point2f(dst_w * 0.5 + dst_dir[0], dst_h * 0.5 + dst_dir[1]);
...
void transform_preds(float* coords, std::vector<float>& center, std::vector<float>& scale, std::vector<int>& output_size, std::vector<int64_t>& dim, float* target_coords) {
...
void get_max_preds(float* heatmap, std::vector<int>& dim, float* preds, float* maxvals, int batchid,  int joint_idx) {
	...
	for (int j = 0; j < dim[1]; j++) {
		float *index = &(heatmap[batchid * num_joints * dim[2] * dim[3] + j * dim[2] * dim[3]]);
...
void get_final_preds(float* heatmap, std::vector<int64_t>& dim, int64_t* idxout, std::vector<int64_t>& idxdim, std::vector<float>& center, std::vector<float> scale, float* preds, int batchid) {
	...
	for (int j = 0; j < dim[1]; ++j) {
		...
		if (px > 1 && px < heatmap_width - 1) {
			float diff_x = heatmap[index + py * dim[3] + px + 1] - heatmap[index + py * dim[3] + px - 1];
			coords[j * 2] += diff_x>0?1:-1 * 0.25;
		}
		if (py>1 && py < heatmap_height - 1) {
			float diff_y = heatmap[index + (py + 1) * dim[3] + px] - heatmap[index + (py - 1) * dim[3] + px];
