...
def parse_dygraph_infos(params, states):
    ...
    for param in params:
        ...
        if len(states_names) != len(param['names']):
            print("***NOTE: dygraph weights recount: {} != {}, {}, {}".format(
                len(states_names),
                len(param['names']), states_names, param['names']))
    ...
    with open("./weight_name_map.txt", 'w') as wf:
        while st_idx < len(static_infos):
            ...
            if dy_idx >= len(dygraph_infos):
                print("static weight not found in dynamic: ",
                      static_infos[st_idx:])
                return
            if info[1] == dygraph_infos[dy_idx][1]:
                print("{:50} matched      {:50}".format(info[0], dygraph_infos[
                    dy_idx][2]))
                wf.write("{:50} {:50}\n".format(info[0], dygraph_infos[dy_idx][
                    2]))
                dy_idx += 1
            else:
                selects = []
                for idx in range(dy_idx + 1, len(dygraph_infos)):
                    if dygraph_infos[idx][1] == info[1]:
                        if st_is_conv_bn_or_fcs[st_idx] > 0:
                            if dy_is_conv_bn_or_fcs[
                                    idx] != st_is_conv_bn_or_fcs[st_idx]:
                                print(
                                    "*****{} matched {}, but ConvBN/FC check failed, {} != {}".
                                    format(dygraph_infos[idx][2], info[
                                        0], dy_is_conv_bn_or_fcs[idx],
                                           st_is_conv_bn_or_fcs[st_idx]))
                                continue
                        selects.append(idx)
                if len(selects) > 1:
                    print("*****match wrong*******", info,
                          dygraph_infos[dy_idx], ", is ConvBN/FC block: ",
                          st_is_conv_bn_or_fcs[st_idx])
                    choose_str = "Please select dygraph weight name for {}:\n".format(
                        info[0])
                    for i, idx in enumerate(selects):
                        choose_str += "\t {}. {}\n".format(
                            i + 1, dygraph_infos[idx][2])
                    ...
                    if len(selects) == 0:
                        with open('./dy_not_match.txt', 'w') as df:
                            for idx in range(dy_idx, len(dygraph_infos)):
                                df.write("{} {}\n".format(dygraph_infos[idx][
                                    2], dygraph_infos[idx][1]))
                        with open('./st_not_match.txt', 'w') as sf:
                            for idx in range(st_idx, len(static_infos)):
                                sf.write("{} {}\n".format(static_infos[idx][0],
                                                          static_infos[idx][1]))
                        print(
                            "ERROR: match wrong, not matched weight saved in dy_not_match.txt and st_not_match.txt"
                        )
                        return
                    idx = selects[0]
                ...
                if st_is_conv_bn_or_fcs[st_idx]:
                    ...
                    for i in range(n):
                        ...
                        dy_is_conv_bn_or_fcs.pop(idx)
                        st_idx += 1
                        print("{:50} matched      {:50}".format(static_infos[
                            st_idx][0], dy_info[2]))
                        wf.write("{:50} {:50}\n".format(static_infos[st_idx][0],
                                                        dy_info[2]))
