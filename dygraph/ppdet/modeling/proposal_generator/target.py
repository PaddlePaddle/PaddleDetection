...
# libra
def libra_sample_pos0(max_classes, pos_inds, num_expected):
...
def libra_subsample_labels0(labels,
                     num_samples,
                     fg_fraction,
                     bg_label=0,
                     use_random=True):
    ...
    if (positive.shape[0] > fg_num) and use_random:
        fg_inds = libra_sample_pos(match_labels_np, fg_inds.numpy(), fg_rois_per_im)

    return fg_inds, bg_inds


def libra_sample_bbox0(matches,
                match_labels,
                gt_classes,
                batch_size_per_im,
                fg_fraction,
...
def libra_generate_proposal_target0(rpn_rois,
                             gt_classes,
                             gt_boxes,
                             batch_size_per_im,
                             fg_fraction,
