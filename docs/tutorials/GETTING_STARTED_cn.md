...
在本演示案例中，假定用户将PaddleDetection的代码克隆并放置在`/home/abc/cv`目录中。用户执行的命令操作均在`/home/abc/cv/PaddleDetection`目录下完成

## 2 准备数据
目前PaddleDetection支持3种数据格式：COCO VOC widerface三种数据格式。
...
## 3 配置文件改动和说明
我们使用`configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml`配置进行训练。
在静态图版本下，一个模型往往可以通过两个配置文件（一个主配置文件、一个reader的读取配置）实现，在PaddleDetection 2.0后续版本，采用了模块解耦设计，用户可以组合配置模块实现检测器，并可自由修改覆盖各模块配置，如下图所示


<center>
<img src="../images/yml.png" width="500" >
</center>
<br><center>配置文件摘要</center></br>


从上图看到`ppyolov2_r50_dcn_365e_coco.yml`配置需要依赖其他的配置文件。在该例子中需要依赖：
...
`ppyolov2_r50_dcn_365e_coco.yml`文件入口

`voc.yml`主要说明了训练数据和验证数据的路径，包括数据格式(coco、voc等)

`ppyolov2_reader.yml` 主要说明了读取后的预处理操作，比如resize、数据增强等等
...
<center><img src="../images/yml2.png" width="1000" ></center>
...
python tools/train.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml
```

* GPU多卡训练
```bash
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 #windows和Mac下不需要执行该命令
python -m paddle.distributed.launch --gpus 0,1,2,3,4,5,6,7 tools/train.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml
```
* Fine-tune其他任务
...
python -m paddle.distributed.launch --gpus 0,1,2,3,4,5,6,7 tools/train.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml \-o pretrain_weights=output/model_final
```

* 模型恢复训练

  在日常训练过程中，有的用户由于一些原因导致训练中断，PaddleDetection提供了恢复训练的方式

```bash
export CUDA_VISIBLE_DEVICES=0
python tools/train.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml -r output/faster_rcnn_r50_1x_coco/10000
 ```
...
* PaddleDetection会默认将训练生成的模型保存在当前`output`文件夹下
 ```bash
export CUDA_VISIBLE_DEVICES=0
python tools/eval.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml -o weights=https://paddledet.bj.bcebos.com/models/ppyolov2_r50_dcn_365e_coco.pdparams
```
* 边训练，边评估

```bash
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7
python -m paddle.distributed.launch --gpus 0,1,2,3,4,5,6,7 tools/train.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml --eval
```
...
```bash
export CUDA_VISIBLE_DEVICES=0
python tools/eval.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml \
             --json_eval \
             -output_eval evaluation/
```
* 上述命令中没有加载模型的选项，实则会加载配置文件中`weights`选项，`weights`表示训练过程中保存的最后一轮模型文件
...
## 6 预测
  ...
  ```bash
  python tools/infer.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml --infer_img=demo/000000570688.jpg -o weights=https://paddledet.bj.bcebos.com/models/ppyolov2_r50_dcn_365e_coco.pdparams
  ```
 * 设置参数预测

  ```bash
  export CUDA_VISIBLE_DEVICES=0
  python tools/infer.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml \
                      --infer_img=demo/000000570688.jpg \
                      --output_dir=infer_output/ \
                      --draw_threshold=0.5 \
                      -o weights=output/ppyolov2_r50_dcn_365e_coco.yml/model_final \
                      --use_vdl=Ture
  ```
...
```bash
python tools/export_model.py -c configs/ppyolo/ppyolov2_r50_dcn_365e_coco.yml --output_dir=./inference_model \
 -o weights=output/ppyolo/ppyolov2_r50_dcn_365e_coco/best_model
```
预测模型会导出到`inference_model/ppyolov2_r50_dcn_365e_coco`目录下，分别为`infer_cfg.yml`, `model.pdiparams`, `model.pdiparams.info`,`model.pdmodel`
