...
|        CropWithSampling        | Several candidate frames are generated according to the scaling ratio and length-width ratio, and then the clipping results that meet the requirements are selected according to the area intersection ratio (IoU) between these candidate frames and the marking frames |
