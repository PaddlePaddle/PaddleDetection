...
In transfer learning, it's needed to load pretrained model selectively. PaddleDetection ignores the parameters which have diffierent shape between model and pretrain\_weights automatically. For example:
...
Besides, the parameters which need to ignore can be specified explicitly as well. Two methods can be used as follwed:
...
2. The parameter fields are set in finetune\_exclude\_pretrained\_params. If the name of parameter matches field (wildcard matching), the parameter will be ignored in loading. One could specify parameter fields arbitrarily.
