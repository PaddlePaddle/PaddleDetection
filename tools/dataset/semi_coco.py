...
def prepare_coco_data(seed=1, percent=10.0, version=2017, seed_offset=0):
    ...
    def _save_anno_sup(name, images, annotations):
        """Save annotation."""
    ...
    def _save_anno_unsup(name, images, annotations):
        """Save annotation."""
        print(">> Processing dataset {}.json saved ({} images {} annotations)".
              format(name, len(images), len(annotations)))
        new_anno = {}
    ...
    anno = json.load(
        open(
            os.path.join(COCOANNODIR, "instances_train{}.json".format(
                version))))
    ...
    _save_anno_sup(save_name, labeled_images, labeled_annotations)
    save_name = "instances_train{version}.{seed}@{tot}-unlabeled".format(
        version=version, seed=seed, tot=int(percent))
    _save_anno_unsup(save_name, unlabeled_images, unlabeled_annotations)
