...
public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    for (int ind = 1; ind < width; ind <<= 1) {
      ...
      MaxOut<T><<<blocks, threads>>>(input_val_data, ind, NC_num, height, width, 3, 0, width - ind, output_data);
...
class LeftPoolGradOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    SliceOnAxis<T><<<blocks, threads>>>(x->data<T>(), NC_num, height, width, 3, width - 1, width, max_val_data);
