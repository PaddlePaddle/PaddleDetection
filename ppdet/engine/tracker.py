...
class Tracker(object):
    ...
    def eval_seq(self,
        ...
        for step_id, data in enumerate(dataloader):
            ...
            dets = outs['dets'].numpy()
            id_feature = outs['id_feature'].numpy()

            width = data['w'].numpy()[0]
            height = data['h'].numpy()[0]
            inp_height = data['image'].numpy().shape[2]
            inp_width = data['image'].numpy().shape[3]
            c = np.array([width / 2., height / 2.], dtype=np.float32)
            s = max(float(inp_width) / float(inp_height) * height, width) * 1.0
            meta = {
                'c': c,
                's': s,
                'out_height': inp_height // self.model.post_process.down_ratio,
                'out_width': inp_width // self.model.post_process.down_ratio
            }

            online_targets = tracker.update(dets, id_feature, meta)
