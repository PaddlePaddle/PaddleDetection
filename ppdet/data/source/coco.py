...
class COCODataSet(DetDataset):
    ...
    def parse_dataset(self):
        ...
        cache_path = os.path.join(
            self.dataset_dir,
            self.cache_path) if self.cache_path else self.cache_path

        if cache_path and os.path.exists(cache_path) and not self.force_reload:
        ...
        records = [record for (record, is_empty) in _records if not is_empty]
        empty_records = [record for (record, is_empty) in _records if is_empty]

        logger.debug('{} samples in file {}'.format(len(_records), anno_path))
        ...
        if cache_path:
            blob = {
                'roidbs': self.roidbs,
                'catid2clsid': self.catid2clsid,
                'cname2cid': self.cname2cid
            }
            pickle.dump(blob, open(cache_path, 'wb'))

    def parse_img(self, img_id, coco):
