...
@register
class DistillGFLLoss(nn.Layer):
    """基于Generalized Focal Loss的蒸馏loss
    """
    def __init__(self):
        super(DistillGFLLoss, self).__init__()
        ...
        self.t_channel_list = [960, 480, 240]
        self.s_channel_list = [768, 384, 192]
        for i in range(self.loss_num):
            distill_loss_module = FGDFeatureLoss(student_channels=self.s_channel_list[i], teacher_channels=self.t_channel_list[i])
            #distill_loss_module = PKDLoss(resize_stu=False)
            distill_loss_module_list.append(distill_loss_module)
        self.distill_loss_module_list = nn.LayerList(distill_loss_module_list)
        # fgd backbone
        distill_loss_module_list_body = []
        self.channel_list_body_s = [256, 512, 1024]
        self.channel_list_body_t = [384, 768, 1536]
        for i in range(self.loss_num):
            distill_loss_module_body = FGDFeatureLoss(student_channels=self.channel_list_body_s[i], teacher_channels=self.channel_list_body_t[i])
            distill_loss_module_list_body.append(distill_loss_module_body)
        self.distill_loss_module_list_body = nn.LayerList(distill_loss_module_list_body)
    ...
    def bbox_loss(self, s_bbox, t_bbox, weight_targets=None):
        ...
        if weight_targets is not None:
            loss_bbox = paddle.sum(
                self.loss_bbox(s_bbox,
                                t_bbox) * weight_targets)
            avg_factor = weight_targets.sum()
            loss_bbox = loss_bbox / avg_factor
        else:
            loss_bbox = paddle.mean(self.loss_bbox(s_bbox,
                                t_bbox))
        return loss_bbox
    ...
    def decoupled_kl_loss(self, l_stu, l_tea, t_label, alpha=1.0, beta=8.0, temperature=1, \
        ...
        if postive_num != 0:
            label_int64_tensor_pos = paddle.to_tensor(label_int64_tensor.numpy()[t_label_numpy!=0])
            ...
            pt_stu, pnt_stu = p_stu[label_int64_tensor_pos], p_stu[1-label_int64_tensor_pos].sum(1)
            pt_tea, pnt_tea = p_tea[label_int64_tensor_pos], p_tea[1-label_int64_tensor_pos].sum(1)
            
            pnct_stu = F.softmax(l_stu[1-label_int64_tensor_pos] / temperature)
            pnct_tea = F.softmax(l_tea[1-label_int64_tensor_pos] / temperature)
    ...
    def distribution_focal_loss(self, pred_corners, target_corners, weight_targets):

        target_corners_label = paddle.nn.functional.softmax(target_corners, axis=-1)
        loss_dfl = paddle.nn.functional.cross_entropy(
            pred_corners, target_corners_label, soft_label=True, reduction='none'
            )
    ...
    def forward(self, teacher_model, student_model):
        ...
        distill_cls_loss.append(
            self.quality_focal_loss(
                    student_distill_pairs['pred_cls_scores'].reshape((-1, student_distill_pairs['pred_cls_scores'].shape[-1])), 
                    teacher_distill_pairs['pred_cls_scores'].detach().reshape((-1, teacher_distill_pairs['pred_cls_scores'].shape[-1])),
        ...
        for i, distill_loss_module in enumerate(self.distill_loss_module_list):
            distill_neck_global_loss.append(
                distill_loss_module(student_fpn_feats[i], teacher_fpn_feats[i], inputs)
            )
        distill_neck_global_loss = paddle.add_n(distill_neck_global_loss)

        #distill_pfi_loss = self.pfi_loss(teacher_model, student_model)
        
        loss = (
            distill_bbox_loss * self.bbox_loss_weight + 
            distill_cls_loss * self.qfl_loss_weight + 
            distill_dfl_loss * self.dfl_loss_weight
        )
        ...
        #return distill_neck_global_loss




@register
class DistillFGDLoss(nn.Layer):
    """基于Generalized Focal Loss（gfl）的蒸馏loss
    """
    def __init__(self, loss_num=3, channel_list=[256,512,1024], loss_weight=1.0):
        super(DistillFGDLoss, self).__init__()
        self.loss_num = 3
        self.loss_weight = loss_weight
        distill_loss_module_list = []
        channel_list = channel_list
        for i in range(self.loss_num):
            distill_loss_module = FGDFeatureLoss(student_channels=channel_list[i], teacher_channels=channel_list[i])
            #distill_loss_module = PKDLoss(resize_stu=False)
            distill_loss_module_list.append(distill_loss_module)
        self.distill_loss_module_list = nn.LayerList(distill_loss_module_list)

    def forward(self, teacher_model, student_model):
        teacher_fpn_feats = teacher_model.head.distill_pairs['teacher_fpn_feats']
        inputs = student_model.inputs
        student_fpn_feats = student_model.head.distill_pairs['student_fpn_feats'] 
        assert 'gt_bbox' in inputs
        distill_loss = 0
        for i, distill_loss_module in enumerate(self.distill_loss_module_list):
            distill_loss += distill_loss_module(student_fpn_feats[i], teacher_fpn_feats[i], inputs)
        return distill_loss * self.loss_weight
...
class FGDFeatureLoss(nn.Layer):
    ...
    def forward(self, stu_feature, tea_feature, inputs):
        ...
        if self.normalize:
            stu_feature, tea_feature = self.norm(stu_feature), self.norm(tea_feature)
...
class DistillPFILoss(nn.Layer):
    ...
    def rm(self, t_score, s_score, assigned_scores, mask_positive_):
        ...
        batch_ind = batch_ind.unsqueeze(-1).tile([1, assigned_scores_ind.shape[1]]).unsqueeze(-1)
        len_ind = len_ind.unsqueeze(0).tile([assigned_scores_ind.shape[0], 1]).unsqueeze(-1)
        assigned_scores_ind = paddle.concat([batch_ind, len_ind, assigned_scores_ind], -1)
        ...
        loss_neg = F.kl_div(paddle.log(s_score), t_score, reduction='none').sum(-1)
    ...
    def pfi(self, t_fpn, s_fpn, t_score, s_score):
        ...
        f_dif = [paddle.abs(t - s).pow(2).mean(1).flatten(1) for t, s in zip(t_fpn, s_fpn)]
...
class PKDLoss(nn.Layer):
    ...
    def forward(self, preds_S, preds_T, input):
        """Forward computation.
        Args:
        ...
        """
        if isinstance(preds_S, paddle.Tensor):
