...
def mean_per_joint_position_error(pred, gt, has_3d_joints):
    """ 
    Compute mPJPE
    pred: shape(batch_size, num_joints, 3)
    """
    ...
    with paddle.no_grad():
        # move to center
        # gt_pelvis = (gt[:, 2, :] + gt[:, 3, :]) / 2 # for Human3.6M
        gt_pelvis = gt[:, 0, :]
...
class Pose3DEval(object):
    ...
    def update(self, inputs, outputs):
        if 'joints_3d' in inputs.keys():
            gt_3d_joints = all_gather(inputs['joints_3d'])  # Tensor
        else:
            gt_3d_joints = all_gather(inputs['kps3d'])
            # reshape
            dims = gt_3d_joints.shape
            gt_3d_joints = paddle.reshape(gt_3d_joints,
                                          (dims[0] * dims[1], dims[2], dims[3]))

        if 'has_3d_joints' in inputs.keys():
            has_3d_joints = all_gather(inputs['has_3d_joints'])
        else:
            has_3d_joints = all_gather(inputs['kps3d_vis'])

        if 'pose3d' in outputs.keys():
            pred_3d_joints = all_gather(outputs['pose3d'])
        else:
            pred_3d_joints = all_gather(outputs['keypoint'])
        ...
        max_value = paddle.max(gt_3d_joints).numpy().tolist()[0]
        if max_value < 200 and max_value > 50:
            # cm --> mm
            self.PAmPJPE.update(PAmPJPE_val * 10., count)  # 毫米为单位
            self.mPJPE.update(mPJPE_val * 10., count)
        else:
            # m-->mm
            self.PAmPJPE.update(PAmPJPE_val * 1000., count)  # 毫米为单位
