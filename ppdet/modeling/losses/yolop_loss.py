...
from ..bbox_utils import make_grid, xywh2xyxy, iou_similarity

__all__ = ['YOLOPLoss']


def decode_yolo(box, downsample_ratio):
    x, y, w, h = box
    ...
    w1 = paddle.exp(w) * downsample_ratio
    h1 = paddle.exp(h) * downsample_ratio

    return [x1, y1, w1, h1]


def bbox_transform(pbox, downsample):
    pbox = decode_yolo(pbox, downsample)
    pbox = xywh2xyxy(pbox)
    return pbox


@register
class YOLOPLoss(nn.Layer):
    ...
    def obj_loss(self, pbox, gbox, pobj, tobj, downsample):
        # pbox
        pbox = decode_yolo(pbox, downsample)
        pbox = xywh2xyxy(pbox)
    ...
    def yolov3_loss(self, p, t, gt_box, downsample, scale=1., eps=1e-10):
        na = 1
        b, c, h, w = p.shape
        ...
        if self.iou_loss is not None:
            ...
            pbox = bbox_transform(box, downsample)
            gbox = bbox_transform(tbox, downsample)
        ...
        box = [x, y, w, h]
        loss_obj = self.obj_loss(box, gt_box, obj, tobj, downsample)
    ...
    def forward(self, inputs, targets):
        ...
        for x, t, downsample in zip(inputs, gt_targets, self.downsample):
            yolo_loss = self.yolov3_loss(x, t, gt_box, downsample,
                                         self.scale_x_y)
