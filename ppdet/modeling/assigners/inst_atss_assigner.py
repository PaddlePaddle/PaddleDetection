...
__all__ = ['InstATSSAssigner']


@register
class InstATSSAssigner(nn.Layer):
    ...
    def __init__(self,
                 ...
                 eps=1e-9,
                 sm_use=False):
        super(InstATSSAssigner, self).__init__()
        self.topk = topk
    ...
    def forward(self,
        ...
        if pred_bboxes is not None:
            # assigned iou
            bbox_ious = batch_iou_similarity(gt_bboxes, pred_bboxes) * mask_positive
            bbox_ious = bbox_ious.max(axis=-2).unsqueeze(-1)

            assert (gt_masks is not None) and (pred_coeffs is not None) \
                   and (mask_feat is not None)
            mask_ious = paddle.zeros_like(bbox_ious, dtype='float32')
            if mask_positive_sum.sum() > 0:
                _assigned_gt_index = mask_positive.argmax(axis=-2)
                scatter_indexs = []
                mask_iou_updates = []
                for i in range(mask_ious.shape[0]):
                    if mask_positive_sum[i].sum() > 0:
                        pos_assigned_gt_index = paddle.masked_select(_assigned_gt_index[i], mask_positive_sum[i] > 0)
                        pos_assigned_mask = paddle.gather(gt_masks[i], pos_assigned_gt_index, axis=0)
                        coeff_mask = mask_positive_sum[i].unsqueeze(-1).tile([1, 32])
                        pos_pred_coeff = paddle.masked_select(pred_coeffs[i], coeff_mask > 0).reshape([-1, 32])
                        pos_pred_mask = paddle.einsum('nq,qhw->nhw', pos_pred_coeff, mask_feat[i])
                        pos_assigned_mask = F.interpolate(
                            pos_assigned_mask.unsqueeze(0),
                            size=pos_pred_mask.shape[-2:],
                            mode='bilinear',
                            align_corners=False).squeeze(0)
                        pos_pred_mask = pos_pred_mask.flatten(1)
                        pos_pred_mask = F.sigmoid(pos_pred_mask)
                        pos_pred_mask = paddle.where(
                            pos_pred_mask > 0.5, 1., 0.).astype('float32')
                        pos_assigned_mask = pos_assigned_mask.flatten(1)
                        pos_assigned_mask = paddle.where(
                            pos_assigned_mask > 0.5, 1., 0.).astype('float32')
                        inter = (pos_pred_mask * pos_assigned_mask).sum(1)
                        union = pos_pred_mask.sum(1) + pos_assigned_mask.sum(1) - inter
                        mask_iou = (inter + 1e-2) / (union + 1e-2)
                        scatter_index = paddle.nonzero(mask_positive_sum[i]).flatten()
                        scatter_index += mask_ious.shape[1] * i
                        scatter_indexs.append(scatter_index)
                        mask_iou_updates.append(mask_iou)
                scatter_indexs = paddle.concat(scatter_indexs, axis=0)
                mask_iou_updates = paddle.concat(mask_iou_updates, axis=0)
                mask_ious = paddle.scatter(mask_ious.flatten(), scatter_indexs, mask_iou_updates)
                mask_ious = mask_ious.reshape(bbox_ious.shape)

            ious = 0.5 * bbox_ious + 0.5 * mask_ious
