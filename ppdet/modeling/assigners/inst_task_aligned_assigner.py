...
__all__ = ['InstTaskAlignedAssigner']
...
@register
class InstTaskAlignedAssigner(nn.Layer):
    ...
    def __init__(self,
                 ...
                 eps=1e-9,
                 is_close_gt=False):
        super(InstTaskAlignedAssigner, self).__init__()
        self.topk = topk
        self.alpha = alpha
        self.beta = beta
        self.eps = eps
    ...
    def forward(self,
        ...
        mask_ious = paddle.zeros_like(alignment_metrics, dtype='float32')
        if mask_positive_sum.sum() > 0:
            ...
            mask_ious = mask_ious.reshape(alignment_metrics.shape)

        assigned_scores = assigned_scores * (0.5 * alignment_metrics + 0.5 * mask_ious)
