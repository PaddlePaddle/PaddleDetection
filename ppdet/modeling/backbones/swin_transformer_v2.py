...
class WindowAttention(nn.Layer):
    ...
    def __init__(self,
        ...
        self.cpb_mlp = nn.Sequential(nn.Linear(2, 512, bias_attr=True),
                                     nn.ReLU(),
                                     nn.Linear(512, num_heads, bias_attr=False))
        
        # get relative_coords_table
        relative_coords_h = paddle.arange(-(self.window_size[0] - 1), self.window_size[0], dtype='float32')
        relative_coords_w = paddle.arange(-(self.window_size[1] - 1), self.window_size[1], dtype='float32')
        relative_coords_table = paddle.stack(
            paddle.meshgrid([relative_coords_h,
                            relative_coords_w])).transpose([1, 2, 0]).unsqueeze(axis=0)  # 1, 2*Wh-1, 2*Ww-1, 2
        ...
        self.relative_coords_table = paddle.sign(relative_coords_table) * paddle.log2(
            paddle.abs(relative_coords_table) + 1.0) / np.log2(8)

        self.register_buffer("relative_coords_table", self.relative_coords_table)
    ...
    def forward(self, x, mask=None):
        ...
        attn = paddle.mm(F.normalize(q, axis=-1), F.normalize(k, axis=-1).transpose([0, 1, 3, 2]))
    ...
    def __init__(self, 
        ...
        self.blocks = nn.LayerList([
            SwinTransformerBlock(dim=dim, 
                                 ...
                                 drop=drop, 
                                 attn_drop=attn_drop,
                                 drop_path=drop_path[i] if isinstance(drop_path, np.ndarray) else drop_path,
    ...
    def forward(self, x, H, W):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                x = paddle.distributed.fleet.utils.recompute((blk, (x, attn_mask)))
            else:
                x = blk(x, attn_mask)
