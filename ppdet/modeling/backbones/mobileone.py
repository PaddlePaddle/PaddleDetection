 ...
 * @Date: 2022-06-21 14:25:00
 * @Last Modified by: nihao
 * @Last Modified time: 2022-06-28 10:27:52
 * @Description: paddle implement of MobileOne block, https://arxiv.org/pdf/2206.04040.pdf, based on https://github.com/DingXiaoH/RepVGG/blob/main/repvgg.py
 */
"""
...
class MobileOneBlock(nn.Layer):
    ...
    def get_equivalent_kernel_bias(self):
        ...
        st1_kernelid, st1_biasid = self._fuse_bn_tensor(self.rbr_identity_st1, kernel_size=self.kernel_size)

        st2_kernel1x1, st2_bias1x1 = self._fuse_bn_tensor(self.point_conv)
        st2_kernelid, st2_biasid = self._fuse_bn_tensor(self.rbr_identity_st2, kernel_size=1)
    ...
    def _pad_1x1_to_3x3_tensor(self, kernel1x1):
        if kernel1x1 is None:
            ...
            return nn.functional.pad(kernel1x1, [padding_size, padding_size, padding_size, padding_size])
    ...
    def _fuse_bn_tensor(self, branch, kernel_size=3):
        ...
        if isinstance(branch, nn.LayerList):
            ...
            kernel_value = paddle.zeros(shape=[self.ch_in, input_dim, kernel_size, kernel_size], dtype='float32')
            if kernel_size > 1:
