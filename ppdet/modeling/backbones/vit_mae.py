...
class Attention(nn.Layer):
    def __init__(self,
        ...
        self.qkv = nn.Linear(
            ...
            dim * 3,
            weight_attr=ParamAttr(learning_rate=lr_factor),
            bias_attr=ParamAttr(learning_rate=lr_factor) if attn_bias else False)
        if qkv_bias:
...
class VisionTransformer2D(nn.Layer):
    ...
    def __init__(self,
        ...
        if use_abs_pos_emb:
            ...
            pos_embed = self.get_2d_sincos_position_embedding(self.patch_h, self.patch_w)
    ...
    def init_weight(self):
        ...
        if pretrained:
            ...
            if pos_embed_name in load_state_dict.keys() and self.use_abs_pos_emb:
