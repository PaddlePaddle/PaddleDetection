...
class FocalModulation(nn.Layer):
    ...
    def __init__(self,
        ...
        self.h = nn.Conv2D(dim, dim, kernel_size=1, stride=1, padding=0, groups=1, bias_attr=True)
...
class PatchEmbed(nn.Layer):
    ...
    def forward(self, x):
        ...
        if W % self.patch_size[1] != 0: # [1, 3, 800, 1199]
            # print('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', W)
            x = F.pad(x, [0, self.patch_size[1] - W % self.patch_size[1], 0, 0])
            W += W % self.patch_size[1]
        if H % self.patch_size[0] != 0:
            # print('yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy', H)
