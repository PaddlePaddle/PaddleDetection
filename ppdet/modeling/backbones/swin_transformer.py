...
class WindowAttention(nn.Layer):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = add_parameter(
            self, paddle.zeros(((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads)))  # 2*Wh-1 * 2*Ww-1, nH
...
class PatchEmbed(nn.Layer):
    ...
    def forward(self, x):
        ...
        if W % self.patch_size[1] != 0:
            x = F.pad(x, [0, self.patch_size[1] - W %
                          self.patch_size[1], 0, 0], data_format='NCHW')
        if H % self.patch_size[0] != 0:
            x = F.pad(x, [0, 0, 0, self.patch_size[0] - H %
                          self.patch_size[0]], data_format='NCHW')
...
class SwinTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        if self.ape:
            ...
            self.absolute_pos_embed = add_parameter(self, paddle.zeros(
