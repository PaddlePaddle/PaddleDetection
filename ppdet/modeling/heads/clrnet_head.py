...
from ppdet.modeling.culane_layers import ROIGather, LinearModule
from ppdet.modeling.culane_layers import SegDecoder
...
class CLRHead(nn.Layer):
    ...
    def __init__(self, num_points=72, prior_feat_channels=64, fc_hidden_dim
        ...
        self.loss = loss
        self.register_buffer(name='sample_x_indexs', tensor=(paddle.
            linspace(start=0, stop=1, num=self.sample_points,dtype=paddle.float32) * self.n_strips).astype(dtype='int64'))
        self.register_buffer(name='prior_feat_ys', tensor=paddle.flip(x=(1 -
            self.sample_x_indexs.astype(
            'float32') / self.n_strips), axis=[-1]))
        self.register_buffer(name='prior_ys', tensor=paddle.linspace(start=
            1, stop=0, num=self.n_offsets).astype('float32'))
        self.prior_feat_channels = prior_feat_channels
        ...
        self.seg_decoder = SegDecoder(self.img_h, self.img_w, self.num_classes, self.prior_feat_channels, self.refine_layers)
        reg_modules = list()
        cls_modules = list()
        ...
        self.roi_gather = ROIGather(self.prior_feat_channels, self.
            num_priors, self.sample_points, self.fc_hidden_dim, self.
            refine_layers)
        self.reg_layers = nn.Linear(in_features=self.fc_hidden_dim,
            out_features=self.n_offsets + 1 + 2 + 1,bias_attr=True)
        self.cls_layers = nn.Linear(in_features=self.fc_hidden_dim,
            out_features=2,bias_attr=True)
        self.init_weights()
    ...
    def pool_prior_features(self, batch_features, num_priors, prior_xs):
        ...
        prior_ys = self.prior_feat_ys.tile(repeat_times=[batch_size *
            num_priors]).reshape([batch_size, num_priors, -1, 1])
        prior_xs = prior_xs * 2.0 - 1.0
        prior_ys = prior_ys * 2.0 - 1.0
        grid = paddle.concat(x=(prior_xs, prior_ys), axis=-1)
        feature = F.grid_sample(x=batch_features, grid=
            grid, align_corners=True).transpose(perm=[0, 2, 1, 3])
        feature = feature.reshape([batch_size * num_priors, self.
            prior_feat_channels, self.sample_points, 1])
        return feature
    ...
    def generate_priors_from_embeddings(self):
        ...
        priors = paddle.zeros((self.num_priors, 2 + 2 + 2 + self.n_offsets), dtype=predictions.dtype)
    ...
    def forward(self, x, inputs=None):
        ...
        for stage in range(self.refine_layers):
            ...
            batch_prior_features = self.pool_prior_features(batch_features[
                stage], num_priors, prior_xs)
            prior_features_stages.append(batch_prior_features)

            fc_features = self.roi_gather(prior_features_stages,
                batch_features[stage], stage)
            # return fc_features
            fc_features = fc_features.reshape([num_priors, batch_size, -1]).reshape(
            ...
            predictions[..., 6:] = (
                ...
                paddle.tan(tran_tensor(predictions[..., 4]) * math.pi + 1e-05))) / (self.img_w - 1)
            ...
            if stage != self.refine_layers - 1:
                ...
                priors_on_featmap = priors.index_select(6 + self.sample_x_indexs,axis=-1)
    ...
    def predictions_to_pred(self, predictions):
        ...
        """
        self.prior_ys = paddle.to_tensor(self.prior_ys,place=predictions.place)
        ...
        for lane in predictions:
            ...
            if start> 0:
                mask = ~((((lane_xs[:start] >= 0.) & (lane_xs[:start] <= 1.)
                       ).cpu().detach().numpy()[::-1].cumprod()[::-1]).astype(np.bool))
            ...
            if len(lane_xs) <= 1:
                continue
            points = paddle.stack(x=(lane_xs.reshape([-1, 1]), lane_ys.
                reshape([-1, 1])), axis=1).squeeze(axis=2)
            lane = Lane(points=points.cpu().numpy(), metadata={'start_x':
                lane[3], 'start_y': lane[2], 'conf': lane[1]})
            lanes.append(lane)
        return lanes
    ...
    def lane_nms_paddle2(self, predictions, scores, nms_overlap_thresh, top_k):
        ...
        while len(condidates) > 0:
            ...
            ious = []
            for i in range(1, len(condidates)):
                ious.append(1-line_iou(condidates[i].unsqueeze(0),condidates[0].unsqueeze(0),img_w=self.img_w, length=15))
            ious = paddle.to_tensor(ious)
    ...
    def get_lanes(self, output, as_lanes=True):
        ...
        for predictions in output:
            ...
            nms_predictions = paddle.concat(x=[nms_predictions[..., :4],
                nms_predictions[..., 5:]], axis=-1)
