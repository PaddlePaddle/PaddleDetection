...
class CenterNetHead(nn.Layer):
    ...
    def forward(self, feat, inputs):
        ...
        size = self.size(feat)
        offset = self.offset(feat) if self.offset_weight else None
        iou = self.iou(feat) if self.iou_weight else None

        if self.training:
            loss = self.get_loss(
                inputs, self.weights, heatmap, size, offset=offset, iou=iou)
            return loss
        else:
            heatmap = F.sigmoid(heatmap)
            head_outs = {'heatmap': heatmap, 'size': size}
            if self.offset_weight:
                head_outs.update({'offset': offset})
            if self.iou_weight:
                head_outs.update({'iou': iou})
            return head_outs

    def get_loss(self, inputs, weights, heatmap, size, offset=None, iou=None):
