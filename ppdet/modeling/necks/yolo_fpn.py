...
from ..architectures.common import Conv, C3, SPP, Concat, parse_model

__all__ = ['YOLOv3FPN', 'YOLOv5FPN', 'PPYOLOFPN', 'PPYOLOTinyFPN', 'PPYOLOPAN']


@register
@serializable
class YOLOv5FPN(nn.Layer):
    ...
    def __init__(self,
                 ...
                 feature_maps=[17, 20, 23], data_format='NCHW'):
        """
        YOLOv3FPN layer
        ...
        self.model, self.save, self.ch = parse_model(
            modules, depth_multiple, width_multiple, in_channels)
        self._out_channels = []
        self.num_blocks = len(in_channels)
        for m in self.model:
            if m.i in feature_maps:
                self._out_channels.insert(0, m.ch)
    ...
    def forward(self, blocks):
        ...
        for i, m in enumerate(self.model):
            if m.f != -1:  # if not from previous layer
                x = blocks[self.save.index(m.f)] if isinstance(m.f, int) else [
                    x if j == -1 else blocks[self.save.index(j)] for j in m.f]  # from earlier layers
            x = m(x)  # run
            if m.i in self.save:
                blocks.append(x)
            if m.i in self.feature_maps:
                feats.insert(0, x)
        return feats
...
class DropBlock(nn.Layer):
    ...
    def forward(self, x):
        if not self.training or self.keep_prob == 1:
            ...
            matrix = paddle.cast(paddle.rand(
                x.shape, x.dtype) < gamma, x.dtype)
