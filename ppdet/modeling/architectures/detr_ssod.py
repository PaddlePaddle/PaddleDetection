...
class DETR_SSOD(MultiSteamDetector):
    def __init__(self, teacher, student, train_cfg=None, test_cfg=None, RTDETRTransformer=None,num_classes=80):
    ...
    def forward_train(self, inputs, **kwargs):
        ...
        if iter_id>self.ema_start_iters:
            ...
            unsup_loss.update({
                'loss':
                paddle.add_n([v*0 for k, v in sup_loss.items() if 'log' not in k])
        })
    ...
    def foward_unsup_train(self, data_unsup_w, data_unsup_s):
        ...
        proposal_score_list = paddle.cast(bbox[:, 1:self.num_classes+1], np.float32)
