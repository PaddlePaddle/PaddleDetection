...
from ppdet.data.transform.keypoint_operators import transform_preds
...
class Hrnet(BaseArch):
    ...
    def _forward(self):
        ...
        if self.training:
            ...
            preds, maxvals = self.post_process(
                hrnet_head_outs, self.inputs, self.backbone, self.hrnet_head, self.flip_perm)
...
def HrnetPostProcess(flip=True, shift_heatmap=True):
    def __init__(self, flip, shift_heatmap):
        self.flip = flip
    ...
    def __call__(self, output, inputs, backbone, head, flip_perm):
        ...
        preds, maxvals = self.get_final_preds(output.numpy(), inputs['center'].numpy(), inputs['scale'].numpy())
