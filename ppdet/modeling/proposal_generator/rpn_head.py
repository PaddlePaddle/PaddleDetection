...
class RPNHead(nn.Layer):
    ...
    def get_loss(self, pred_scores, pred_deltas, anchors, inputs):
        ...
        # inside_flags = inside_flags.logical_and(anchors[:, 2] < inputs['im_shape'][0][1])
        # inside_flags = inside_flags.logical_and(anchors[:, 3] < inputs['im_shape'][0][0])
        ...
        # score_tgt, bbox_tgt, loc_tgt, norm = self.rpn_target_assign(inputs, anchors, inside_flags=inside_flags)
